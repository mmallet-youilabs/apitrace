...
dumpTextureTargetParameters(StateWriter &writer, Context &context, GLenum target, GLuint texture)
{
    ...
    glGetTexParameteriv(target, GL_TEXTURE_TARGET, &texture_target);
            if (glGetError() != GL_NO_ERROR) {
                flushErrors();
            } else {
                writer.beginMember("GL_TEXTURE_TARGET");
    dumpEnum(writer, texture_target);
                writer.endMember();
            }
        }
...
void dumpParameters(StateWriter &writer, Context &context)
{
    ...
    writer.writeInt(debug_next_logged_message_length);
        ...
        {
            flushErrors();
    GLint reset_notification_strategy = 0;
    glGetIntegerv(GL_RESET_NOTIFICATION_STRATEGY, &reset_notification_strategy);
            if (glGetError() != GL_NO_ERROR) {
                flushErrors();
            } else {
                writer.beginMember("GL_RESET_NOTIFICATION_STRATEGY");
    dumpEnum(writer, reset_notification_strategy);
                writer.endMember();
            }
        }
    ...
    glGetIntegerv(GL_MAX_VERTEX_ATTRIB_STRIDE, &max_vertex_attrib_stride);
            if (glGetError() != GL_NO_ERROR) {
                flushErrors();
            } else {
                writer.beginMember("GL_MAX_VERTEX_ATTRIB_STRIDE");
    writer.writeInt(max_vertex_attrib_stride);
                writer.endMember();
            }
        }
    ...
    writer.writeInt(max_tess_evaluation_uniform_blocks);
        ...
        {
            flushErrors();
    GLint window_rectangle_mode_ext = 0;
    glGetIntegerv(GL_WINDOW_RECTANGLE_MODE_EXT, &window_rectangle_mode_ext);
            if (glGetError() != GL_NO_ERROR) {
                flushErrors();
            } else {
    ...
    dumpEnum(writer, window_rectangle_mode_ext);
        ...
        {
            flushErrors();
    GLint max_window_rectangles_ext = 0;
    glGetIntegerv(GL_MAX_WINDOW_RECTANGLES_EXT, &max_window_rectangles_ext);
            if (glGetError() != GL_NO_ERROR) {
                flushErrors();
            } else {
    ...
    writer.writeInt(max_window_rectangles_ext);
        ...
        {
            flushErrors();
    GLint num_window_rectangles_ext = 0;
    glGetIntegerv(GL_NUM_WINDOW_RECTANGLES_EXT, &num_window_rectangles_ext);
            if (glGetError() != GL_NO_ERROR) {
                flushErrors();
            } else {
    ...
    for (GLint index = 0; index < max_lights; ++index) {
        ...
        if (glIsEnabled(light)) {
            std::string name = std::string("GL_LIGHT") + std::to_string(index);
